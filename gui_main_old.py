    ...
    def toggle_connection(self):
        ...
        if self.printer.is_inner_connected() or self.printer.is_outer_connected():
            try:
                ...
                self.printer.disconnect_all()  # Fully releases COM ports
                ...
                disconnected_ports = []
                if inner_was_connected:
                    disconnected_ports.append("Inner Printer")
                if outer_was_connected:
                    disconnected_ports.append("Outer Printer")

                self.log(f"🔌 Disconnected {' & '.join(disconnected_ports)} and released COM ports.")
                messagebox.showinfo("Disconnected", "✅ All active printer ports have been safely disconnected.")
            except Exception as e:
        ...
        if inner_success:
            ...
            self.log(f"📦 Inner printer connected to {inner_port}")
        else:
        ...
        if outer_success:
            ...
            self.log(f"📦 Outer printer connected to {outer_port}")
        else:
    ...
    def check_serial_in_db(self):
        ...
        if self.unit_count % self.units_per_innerbox == 0:
            if not self.printer.is_inner_connected():
                ...
                sku = "43000166102"
                barcode = f"{batch_code}{current_innerbox:03d}"
